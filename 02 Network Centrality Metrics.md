...
called the *node degree* and is denoted as $\delta(v_i)$. There are three coefficients that are used to describe networks: The number of nodes, the number of edges, and the average node degree, $<\delta>$. The following is an example of a very simple network. 

This network has four nodes that compose the vertex set $(v_1, v_2, v_3, v_4) \subset V(G)$ and three edges that make up the edge set $[(e_{1,4}),(e_{2,4}),(e_{3,4})] \subset E(G)$. 
